...
from src.data.dataset import ColliderMLHits, ContrastiveLearningDatasetPlanar, ContrastiveLearningGraphDatasetPlanar, EventGraphBuilder, projected_hits
...
VALID_CHANNELS = ("ggf", "ttbar", "dihiggs")
...
parser.add_argument("--channel-a", required=True, choices=VALID_CHANNELS, help="First ColliderML process")

parser.add_argument("--channel-b", required=True, choices=VALID_CHANNELS, help="Second ColliderML process")

parser.add_argument("--events-per-channel", default=2500, type=int, help="Number of events loaded from each process")
...
parser.add_argument('--energy-noise', default=1e-4, type=float, help='to raw hit energy in GeV')
...
parser.add_argument("--crop-fraction", default=0.5, type=float, help="Spatial crop fraction; 0 disables cropping")
...
parser.add_argument("--train-augmentation-order", default="energy_noise+rotate+crop", type=str, help=(
...
def load_channel(channel, events_per_channel, data_dir):
    ...
    calo_hits = frames["calo_hits"]

    if len(calo_hits) != events_per_channel:
        raise RuntimeError(
            f"Requested {events_per_channel} {channel} events, "
            f"but loaded {len(calo_hits)}."
        )
    print(
        f"Loaded {len(calo_hits)} events "
        f"from channel {channel}."
    )
    return calo_hits

channel_a_hits = load_channel(args.channel_a, args.events_per_channel, data_dir)
channel_b_hits = load_channel(args.channel_b, args.events_per_channel, data_dir)
# Concatenate
#combined = pl.concat([calo_hits1, calo_hits2, calo_hits3])
combined = pl.concat([channel_a_hits, channel_b_hits])
...
def build_train_augmentation(args, stage):
    transform_factories = {
    ...
    print(f"{stage} augmentation pipeline: " + (" -> ".join(transform_names) if transform_names else "none"))

    return Compose(transforms)

train_augment = build_train_augmentation(args, stage="Training")
eval_augment = build_train_augmentation(args, stage="Evaluation")
...
VALID_PROJECTIONS = ("eta-phi", "x-y", "z-rho", "z-phi")

def compute_projection_stats(train_hits, projections, seed, grid_size=32):
    """
    Compute feature normalization statistics for one
    or more planar projections using only clean
    training events.
    """

    unknown = [projection for projection in projections if projection not in VALID_PROJECTIONS]

    if unknown:
        raise ValueError(
            f"Unknown projections: {unknown}. "
            f"Valid projections: {VALID_PROJECTIONS}"
        )

    stats_base = ColliderMLHits(train_hits, split=None, shuffle_files=False, log=False, seed=seed,)

    scalers = {projection: StandardScaler() for projection in projections
    }

    node_counts = {projection: 0 for projection in projections
    }

    for event_dict in stats_base:
        raw_hits = event_dict["calo_hit_features"]

        projection_dict = projected_hits(raw_hits, grid_size=grid_size, typ="hits")

        for projection in projections:
            nodes = projection_dict[projection]
            ...
            if not np.isfinite(nodes).all():
                raise ValueError(
                    f"NaN or Inf found in "
                    f"{projection} projection."
                )

            scalers[projection].partial_fit(nodes)

            node_counts[projection] += len(nodes)

    means = []
    stds = []

    for projection in projections:
        if node_counts[projection] == 0:
            raise RuntimeError(
                f"No nodes found for projection "
                f"{projection}."
            )

        means.append(scalers[projection].mean_.astype(np.float64))

        stds.append(scalers[projection].scale_.astype(np.float64))

    return means, stds, node_counts

PROJS = ["eta-phi"]

MEANS, STDS, NODE_COUNTS = compute_projection_stats(train_hits=train_hits, projections=PROJS, seed=SEED, grid_size=32)
...
dataset_train = ContrastiveLearningGraphDatasetPlanar(ContrastiveLearningDatasetPlanar(dataset_train_base, MEANS, STDS,  train_augment, projs=PROJS, grid_size=32, typ='hits'), builder=EventGraphBuilder(radius=r))

dataset_val = ContrastiveLearningGraphDatasetPlanar(ContrastiveLearningDatasetPlanar(dataset_val_base, MEANS, STDS, eval_augment, projs=PROJS, grid_size=32, typ='hits'), builder=EventGraphBuilder(radius=r))

dataset_test = ContrastiveLearningGraphDatasetPlanar(ContrastiveLearningDatasetPlanar(dataset_test_base, MEANS, STDS, eval_augment, projs=PROJS, grid_size=32, typ="hits"), builder=EventGraphBuilder(radius=r))
