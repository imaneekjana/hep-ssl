...
from src.data.augmentation_physics_v2 import *
from src.data.dataset_physics_v2 import ColliderMLHits, ContrastiveLearningDatasetPlanar, ContrastiveLearningGraphDatasetPlanar, EventGraphBuilder
...
parser.add_argument('--beam-reflection-prob', default=0.5, type=float,
                    help='Probability of exchanging the two beam directions')

parser.add_argument('--soft-max-fraction', default=0.01, type=float,
                    help='Maximum fraction of event energy removed by soft masking')

parser.add_argument('--soft-cell-fraction', default=1e-3, type=float,
                    help='Maximum E_cell / E_event for a soft-mask candidate')

parser.add_argument('--share-max-fraction', default=0.02, type=float,
                    help='Maximum event-energy fraction redistributed locally')
...
parser.add_argument("--train-augmentation-order", default="none", type=str, help=(
    "Physics-aware augmentation pipeline. Join names with '+'. "
    "Available transforms: beam_reflection, phi_cyclic, soft_mask, "
    "local_cell_sharing. Use 'none' to disable all augmentations."
))
...
def build_augmentation_pipeline(args):
    raw_factories = {
        "beam_reflection": lambda: RandomBeamReflectionZ(
            probability=args.beam_reflection_prob
        ),
    }
    grid_factories = {
        "phi_cyclic": lambda: RandomCyclicPhiRoll(phi_axis=0),
        "soft_mask": lambda: SoftCellMask(
            max_removed_fraction=args.soft_max_fraction,
            cell_fraction_threshold=args.soft_cell_fraction,
        ),
        "local_cell_sharing": lambda: LocalCellEnergySharing(
            max_moved_fraction=args.share_max_fraction,
            transfer_fraction_range=(0.1, 0.3),
        ),
    }
    valid_names = set(raw_factories) | set(grid_factories)
    ...
    unknown_names = [name for name in transform_names if name not in valid_names]

    if unknown_names:
        valid_names_text = ", ".join(sorted(valid_names))
        raise ValueError(
            f"Unknown augmentation names: {unknown_names}. "
            f"Valid names are: {valid_names_text}"
        )
    ...
    raw_names = [name for name in transform_names if name in raw_factories]
    grid_names = [name for name in transform_names if name in grid_factories]
    raw_transforms = [raw_factories[name]() for name in raw_names]
    grid_transforms = [grid_factories[name]() for name in grid_names]

    print(
        "Raw-hit augmentation pipeline: "
        + (" -> ".join(raw_names) if raw_names else "none")
    )
    print(
        "Cell-grid augmentation pipeline: "
        + (" -> ".join(grid_names) if grid_names else "none")
    )

    return Compose(raw_transforms), ComposeGrid(grid_transforms)

train_raw_augment, train_grid_augment = build_augmentation_pipeline(args)

# Validation and contrastive test loss must evaluate the same pretext task
# that each model was trained on, rather than the old fixed baseline recipe.
eval_raw_augment, eval_grid_augment = build_augmentation_pipeline(args)
...
dataset_train = ContrastiveLearningGraphDatasetPlanar(ContrastiveLearningDatasetPlanar(dataset_train_base, MEANS, STDS, train_raw_augment, grid_transform=train_grid_augment, projs=['eta-phi'], grid_size=32, typ='hits'), builder=EventGraphBuilder(radius=r))

dataset_val = ContrastiveLearningGraphDatasetPlanar(ContrastiveLearningDatasetPlanar(dataset_val_base, MEANS, STDS, eval_raw_augment, grid_transform=eval_grid_augment, projs=['eta-phi'], grid_size=32, typ='hits'), builder=EventGraphBuilder(radius=r))

dataset_test = ContrastiveLearningGraphDatasetPlanar(ContrastiveLearningDatasetPlanar(dataset_test_base, MEANS, STDS, eval_raw_augment, grid_transform=eval_grid_augment, projs=["eta-phi"], grid_size=32, typ="hits"), builder=EventGraphBuilder(radius=r))
