...
dataset_train = ContrastiveLearningGraphDataset(ContrastiveLearningDataset(dataset_train, augment), builder=EventGraphBuilder(radius=3))
dataset_val = ContrastiveLearningGraphDataset(ContrastiveLearningDataset(dataset_val, augment), builder=EventGraphBuilder(radius=3))



train_loader = DataLoader(dataset_train, batch_size=64, drop_last=True)
val_loader   = DataLoader(dataset_val,   batch_size=64, drop_last=True)
...
with torch.cuda.device(args.gpu_index):
    ...
    simclr.train(train_loader, val_loader, off=0, save_model=True, folder = '/global/cfs/cdirs/m4474/aneek/particlemind_aneek/saved_models_colliderml/from_job/', wandb_=True, key='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', name='Test-run-CaloHitsGNN-ColliderML-mixed-ttbar-dihiggs-jobfile')
